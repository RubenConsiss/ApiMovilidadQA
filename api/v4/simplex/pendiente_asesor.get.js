...
exports.simplex_pendiente_asesor_get = async (req, res, next) => {
  if (!db) {
  ...
  if (!results.length) {
    ...
    const qryInsert = `INSERT INTO ${table} (anio, mes, semana_del_anio, checks, crplaza, crtienda) VALUES(?)`
    ...
    rowsParsed = rows.map(row => {
      return {
        ...
        checks: JSON.parse(row.checks).map(check => {
          ...
          for (let index = 0; index < check.pendientes.length; index++) {
            if ('lider' in check.pendientes[index] &&
              'comentarios' in check.pendientes[index] &&
              'status' in check.pendientes[index] &&
              'reasignar_a' in check.pendientes[index] &&
              'solicitante' in check.pendientes[index]
            ) {
              pendientes.push({
                ...check.pendientes[index],
                id: count += 1
              })
            } else {
              pendientes = []
